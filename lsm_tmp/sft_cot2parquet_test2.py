...
def load_examples(path: str) -> List[Dict[str, str]]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # {"role": "user", "content": PROMPT_USER_START.format(data.get("query",""))}
            # ]

            question = data.get("query", "")
            #user_content = PROMPT_USER_START.format(question=question)
