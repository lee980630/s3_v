...
class LLMGenerationManager:
    ...
    def run_llm_loop(self, gen_batch, initial_input_ids: torch.Tensor) -> Tuple[Dict, Dict]:
        ...
        for step in range(self.config.max_turns):
            ...
            # 🚦 바로 여기입니다! dones가 생성된 직후에 마스크 업데이트 로직을 실행합니다.
            # 1. '부분 결과(dones)'를 텐서로 변환합니다.
            curr_done_mask = torch.tensor([not done for done in dones], dtype=torch.bool).to(active_mask.device)

            # 2. 업데이트를 위해 기존의 '전체 명단'을 복사합니다.
            updated_active_mask = active_mask.clone()

            # 3. '전체 명단'에서 현재 활성화된 작업들의 원래 위치(인덱스)를 찾아냅니다.
            active_indices = torch.where(active_mask)[0]

            # 4. 찾아낸 인덱스를 사용해, '부분 결과'를 '전체 명단'의 올바른 위치에 덮어씌웁니다.
            updated_active_mask[active_indices] = curr_done_mask

            # 5. 최종적으로 안전하게 업데이트된 전체 마스크를 할당합니다.
            ...
            #added
            # --- [수정 시작] ---
            # 1. '리본'을 120개 준비하기 위한 빈 목록(템플릿)을 만듭니다.
            #    (''는 비활성화된 자리를 의미하는 빈 문자열입니다.)
            full_batch_size = active_mask.shape[0]
            padded_next_obs = [''] * full_batch_size

            # 2. 현재 활성화된(True) 항목들의 원래 위치(인덱스)를 가져옵니다.
            active_indices = torch.where(active_mask)[0].tolist()

            # 3. 활성화된 위치에만 'next_obs' 결과를 채워 넣습니다.
            for idx, obs in zip(active_indices, next_obs):
                padded_next_obs[idx] = obs

            # 4. 개수를 맞춘 `padded_next_obs`를 사용해 다음 토큰 ID를 생성합니다.
            next_obs_ids, next_obs_str, next_obs_multi_modal_data, next_obs_multi_modal_inputs = self._process_next_obs(padded_next_obs, rollings)
            # --- [수정 끝] ---
    ...
    def execute_predictions(self, predictions: List[str], uids: np.ndarray, pad_token: str, active_mask=None, do_search=True) -> List[str]:
        ...
        for i, (action, content, active) in enumerate(zip(cur_actions, contents, active_mask)):
            # --- [수정된 부분] --- len(search_results) == 0 오류 해결
            # active가 True이고 action이 'search'인 경우에만 요청 목록에 추가
        ...
        if do_search:
            if len(search_requests) > 0:
                ...
                for i in range(0, len(search_requests), batch_size):
                    ...
                    #response = requests.post(self.config.search_url, params={"requests": json.dumps(batch_reqs)}) #수정 post 방식 적용
