...
#     #def execute_predictions(self, predictions: List[str], pad_token: str, active_mask=None, do_search=True) -> List[str]: #수정 제거, uid 넘기기
#     def execute_predictions(self, predictions: List[str], uids: np.ndarray, pad_token: str, active_mask=None, do_search=True) -> List[str]: #수정 추가 uid 넘기기
...
#         #search_queries = [content for action, content in zip(cur_actions, contents) if action == 'search'] #수정 제거 , uid 받기
...
#                     "query": content,
#                     "id": uids[i]  # 1단계에서 전달받은 uids 사용
#                 })        
#         #--끝--

#         if do_search:
#             #if len(search_queries) > 0: #수정 uid 넘기기 제거
#             if len(search_requests) > 0: #수정 uid 넘기기 추가
#                 batch_size = 100
#                 search_results = []
#                 #for i in range(0, len(search_queries), batch_size):#수정 uid 넘기기 제거
#                 for i in range(0, len(search_requests), batch_size): #수정 uid 넘기기 추가
#                     #batch_queries = search_queries[i:i + batch_size] #수정 uid 넘기기 제거
#                     batch_reqs = search_requests[i:i + batch_size] #수정 uid 넘기기 추가
#                     #response = requests.get(self.config.search_url, params={"queries": batch_queries}) #수정 제거 uid 넘기기
#                     response = requests.get(self.config.search_url, params={"requests": json.dumps(batch_reqs)}) #수정 추가 uid 넘기기
...
# ###########추가 id#######
#         search_consumption_count = 0  # 소비 카운터
#         print(f"\n--- 🕵️ 소비 루프 추적 시작 🕵️ ---")
#         print(f"루프 시작 전 search_results 길이: {len(search_results)}")
...
#             if action == 'search':
#                 print(f"  - 루프 {i}: 'search' 액션 발견. 결과 소비 시작.")
#                 print(f"    - pop 실행 전 search_results 길이: {len(search_results)}")
#                 search_results.pop(0)
#                 print(f"    - pop 실행 후 search_results 길이: {len(search_results)}")
#                 search_consumption_count += 1
        
#         print(f"--- 🕵️ 소비 루프 추적 종료 🕵️ ---\n")
# ###########
#         for i, (action, active) in enumerate(zip(cur_actions, active_mask)):
...
#         # 기존의 최종 점검 디버깅 코드는 그대로 둡니다.
#         print("\n--- 🐛 최종 점검 디버깅 🐛 ---")
#         print(f"서버에 보낸 검색 요청 개수: {len(search_requests)}")
#         print(f"서버에서 받은 결과 개수 (남아있는 개수): {len(search_results)}")
#         print(f"실제로 소비한 결과 개수: {search_consumption_count}")
#         print(f"남아있는 결과 내용: {search_results}")
#         print("----------------------------\n")
#         #######
