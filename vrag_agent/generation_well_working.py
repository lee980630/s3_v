    ...
    def execute_predictions(self, predictions: List[str], uids: np.ndarray, pad_token: str, active_mask=None, do_search=True) -> List[str]:
        ...
        if do_search:
            if len(search_requests) > 0:
                ...
                # =======================================================
                # [추가할 코드] 요청과 결과의 개수를 비교하기 위해 출력합니다.
                # =======================================================
                print("--- 🔬 Verifying Request/Response Counts ---")
                print(f"Number of Sent Requests: {len(search_requests)}")
                print(f"Number of Received Results: {len(search_results)}")
                print("------------------------------------------")
                # =======================================================
